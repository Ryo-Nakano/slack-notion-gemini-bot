...
Gemini API は MCP にネイティブ非対応のため、以下のエージェントループを自前実装する。（※実装時の注意点やベストプラクティスについては `workspace/MCP_BRIDGE_BEST_PRACTICES.md` を参照すること）
...
Botがユーザーの曖昧な指示から意図通りにNotionを検索し、正確な回答を生成するためには、単純にツールを渡すだけでなく、LLMに対する「Notionへの検索ガイドライン」を綿密に設計する必要があります。

- **ツール定義のオーバーライド**:
  MCPサーバーが提供するツールのJSON SchemaをGemini向けに変換する際、`description` をより具体的で詳細なテキストに上書きします（例: `query_database` は構造化DBに対するプロパティ検索用、`search` は横断的なキーワード検索用、など）。LLMはこれを最大の根拠としてツールを選択します。
- **システムプロンプト (systemInstruction) の設計**:
  - **Notionのデータ構造の提示**: どのDBにどんなプロパティ（日付、タグ、担当者など）があるかを教え、精度の高いクエリを作らせます（必要に応じてインメモリキャッシュを用い、スキーマ情報を動的に取得・保持することを検討）。
  - **質問タイプ別の検索戦略**: 「〇〇の議事録」ならDB検索、「〇〇について」なら全文検索など、定石となる手順を明記します。
  - **ReActパターンの導入**: ツールを実行する前に、「何を探すべきか」「どのツールを使うか」を一度思考させるプロセスを促し、ハルシネーションや不適切なツール実行を防ぎます。
  - **Few-shotサンプルの追加**: 具体的な思考プロセスとツール利用の例文を含めることで、期待する挙動に安定させます。
- **検索の失敗とフォールバック**:
  検索してヒットしなかった場合に、すぐに「ありませんでした」と諦めるのではなく、キーワードを変えて再検索するよう促したり、どうしても見つからない場合は正直に伝えるように指示します。
...
> **注意**: 実装するときには `workspace/MCP_BRIDGE_BEST_PRACTICES.md` に記載のベストプラクティス内容を参照すること。
